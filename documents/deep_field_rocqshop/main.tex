...
\title{Improving the Ring and Field tactics to ease mathematical learning\\
\small A solution involving an oracle, foreign function calls, and meta-programming}
\author{Yves Bertot}
\date{July 2026}
\mode<presentation>
\begin{document}
...
\begin{itemize}
  \item {\tt field} does it and requires the proofs of
  \(\pi ^ 2 + \pi ^ 2 \neq 0\) and \(\pi \neq 0\)
\end{itemize}
\item Given the goal \(e^{\frac \pi {\pi ^ 2 * \pi ^ 2}} = e^{\frac 1{2* pi}}\)
\begin{itemize}
\item {\tt field} fails
\end{itemize}
