...
\end{itemize}
\item By contrast \(e^{\left(\pi \times (\pi \times 1) + \pi * \pi\right)} = e ^ {2 \times (\pi \times (\pi \times 1))}\) is
{\tt X = Y},
\begin{itemize}
\item {\tt X} instanciated with \(e^{\left(\pi \times (\pi \times 1) + \pi * \pi\right)}\)\\
{\tt Y} instanciated with \(e ^ {2 \times (\pi \times (\pi \times 1))}\)
...
\item collect all formulas appearing as equality members or function arguments
\begin{itemize}
\item \(cos (x + cos (x + x))\)
\item \(x + cos (x + x)\) 
\item \(x + x\)
...
\item The rest of this talk describe the infrastructure
\end{itemize}
\end{frame}
\begin{frame}
